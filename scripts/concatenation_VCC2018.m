...
filelist=dir(['../dataset/features/US_102/Normal/*.mcc']);
filelist1=dir(['../dataset/features/US_102/Whisper/*.mcc']);
...
for index=1:length(filelist)
    ...
    fid=fopen(['../dataset/features/US_102/Normal/',filelist1(index).name]);
    x=fread(fid,Inf,'float');
    x=reshape(x,dim,length(x)/dim);

    fid1=fopen(['../dataset/features/US_102/Whisper/',filelist1(index).name]);
